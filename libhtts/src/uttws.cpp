...
BOOL UttCellWS::isStartOf(UttLevel range) const
{
	...
	if (inURange(ULEVEL_PARAGRAPH, range) && (paragraph_position == 1 || paragraph_position == -1)) return TRUE;  //inaki (-1: if there is only one sentence)
	...
	return UttCell::isStartOf(delURange(addURange(ULEVEL_PARAGRAPH | ULEVEL_SENTENCE | ULEVEL_WORD | ULEVEL_PHRASE | ULEVEL_PAUSE | ULEVEL_AGRP | ULEVEL_TNOR | ULEVEL_FGRP), range)); //inaki
#else
...
{
	fprintf(f,"/");
	if(getParagraphPosition() == 0)//inaki
		fprintf(f," /");
	else
		fprintf(f,"%d/",getParagraphPosition());//inaki
