...
-------         --------        --------  ----------
1.0.1	 09/07/13  Inaki	  Incluir declinaciones en palabras con transcripción en diccionario (tf_mrk)
...
en todas sus formas*/
	if (encontrado==FALSE) {
		...
		//////////////////////////////////
		switch (u.getHDicDB()->query(u.cell(p).getHDicRef(),HDIC_QUERY_EU_TF_MRK)) {//INAKI transcripcion en diccionario
            case 1:
				//comprobar si lo que no coincide es declinación
				//Es declinación?
				...
				BOOL isDec = u.getHDicDB()->query(myhdic, HDIC_QUERY_EU_DEC);
				...
				//	fprintf(stderr,"len2=%d\tstrlen(decli)=%d\n", len2, strlen(decli));
					if (len2 == 0){
