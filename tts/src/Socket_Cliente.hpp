...
class ClientConnection : public Connection{
	public:
		ClientConnection();
		ClientConnection(const Options op);
		int OpenInetConnection(const char *IPServidor, const int PuertoServicio);
		//int SendUtt(const char *utt);
		//int SendLanguage(const char *lan);
		int SendOptions();
		void WriteOptions(const Options op);
		//int ReadFile(FILE* fd);
		void CloseConnection();
		int ObtainSSocket(void){return socket_server;}
	private:	
		int socket_server;
		Options opciones;
