...
//INAKI
CtI CruTxt::setParagraphPosition( CtI p, int number)
{
#ifdef HTTS_DEBUG
        if (!p) htts_error("CruTxt::setParagraphPosition() - Trying to read null CtI");
#endif
  cell(p).paragraph_position = number;
  return p;
}
...
//INAKI
int CruTxt::getParagraphPosition( CtI p)
{
#ifdef HTTS_DEBUG
        if (!p) htts_error("CruTxt::getParagraphPosition() - Trying to read null CtI");
#endif
  return cell(p).paragraph_position;
}
