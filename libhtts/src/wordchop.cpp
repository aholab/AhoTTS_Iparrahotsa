...
CtI WdChop::preChop(pCHAR fltbuff)
{
	...
	for (i=0; i<len; i++)
	{
		//fprintf(stderr,"i=%d\tfltbuff=%c\tchtype=%d\tpat=%c\n", i, fltbuff[i], getchtype(fltbuff[i]), getpatsym(getchtype(fltbuff[i])) );
		chtype = getchtype(fltbuff[i]);
		///////////////////////////////////////////////////////////////////////////////////////////////////
		//inaki si es "-" entre números los hacemos desaparecer: resultados deportivos
		if (fltbuff[i]=='-' && i>0 && getpatsym(getchtype(fltbuff[i-1])) == 'n' && i+1< len && getpatsym(getchtype(fltbuff[i+1])) == 'n'){
			wordStarted = FALSE;
			wordbuff[j]='\x0';
			pattern[k]='\x0';
			j=0;
		...
		///////////////////////////////////////////////////////////////////////////////////////////////////
		//inaki en formas tipo l.l-l (Berasaluze II.a-Albisu.... quitamos el -
		if (fltbuff[i]=='-' && i>2 && getpatsym(getchtype(fltbuff[i-1])) == 'l' && (fltbuff[i-2]=='.' || fltbuff[i-3]=='.' ) && i+1< len && getpatsym(getchtype(fltbuff[i+1])) == 'l'){
			wordStarted = FALSE;
			wordbuff[j]='\x0';
			pattern[k]='\x0';
			j=0;
		...
		else
		{
			...
			//inaki
			if(fltbuff[i] == 10 || fltbuff[i] == 13) { //Carriage Return
				if(p)
					ct.setParagraphPosition(p,1);
			}
		}
	}
