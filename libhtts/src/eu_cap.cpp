...
----------------------------------------------
0.0.6	05/05/13	Inaki		t+c+h -> tx (Por ejemplo Etcheberry)
...
fin:
	...
	for(i=0,j=0,temp=' ';i<strlen(word);i++)
	{
		...
		////////////////////////////////////////////
		//inaki : t+c+h -> tx  (Etcheberry -> Etxeberry)
		else if(word[i]=='t'){//inaki t+c+h -> tx
			if(i+2 < strlen(word) && word[i+1] == 'c' && word[i+2] == 'h'){
				//eliminamos la t y convertimos c+h en tx
				char *tmp = strdup(word);//inaki, usamos array temporal para evitar problemas
				tmp[i+1] = 't';
				tmp[i+2] = 'x';
				strcpy(&(word[i]), &(tmp[i+1]));//inaki
				free(tmp);//inaki
				i--;
			}
			/*else if(i+1 < strlen(word) && word[i+1] == 'c' && IparraldeVoice){//inaki t+c -> tz
				word[i+1] = 'z';
				j++;
			}*/
			else
