...
enum {
	...
	ULEVEL_FGRP     =0x0040,
	ULEVEL_TNOR     =0x0080,*/
	ULEVEL_SENTENCE =0x0004,
	ULEVEL_PHRASE   =0x0008,
	ULEVEL_WORD     =0x0010,
	ULEVEL_PAUSE    =0x0020,
	ULEVEL_AGRP     =0x0040,
	ULEVEL_FGRP     =0x0080,
...
el comienzo de una frase implica comienzo de palabra, etc etc. */
	URANGE_PARAGRAPH = URANGE_UTT | ULEVEL_PARAGRAPH,
	//URANGE_SENTENCE = URANGE_UTT | ULEVEL_SENTENCE,
	URANGE_SENTENCE = URANGE_PARAGRAPH | ULEVEL_SENTENCE,
...
enum Sentence {
  ...
  USENTENCE_PAR=')', USENTENCE_PAROPEN='(', USENTENCE_MARKS='\"'
};
...
private:
	int paragraph_position; //inaki: number of sentence in paragraph (the last one has a minus sign)
...
public:
	...
	VOID setParagraphPosition( int number){ paragraph_position=number;} //inaki
...
#endif
	int getParagraphPosition( VOID ) const { return paragraph_position; }//inaki
...
public:
	...
	UTT_LEVELSHORTCUTS(paragraph,ULEVEL_PARAGRAPH);//inaki
...
ojo, que al borrar celda se libera esta cadena (con free)!
