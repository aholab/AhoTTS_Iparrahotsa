...
-------------------------------------------------------------
0.1.2	 05/06/13  Inaki	 Números en dialecto de Iparralde
...
pCHAR tens2468Table_iparralde[]={
      (char*)"hogoi", (char*)"berrogoi", (char*)"hirutan hogoi", (char*)"lauretan hogoi"
};
	...
	while (num)	{
		if (num<=9) {
			...
			if (num<=19) {
				...
				if(IparraldeVoice)//zenbakiak
					p = ct.insafter(p, tens2468Table_iparralde[(UINT)(num/20-1)], FALSE, emotion, emo_intensity);//inaki
				else
