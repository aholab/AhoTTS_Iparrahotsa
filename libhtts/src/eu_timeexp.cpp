...
-------  --------  --------  -----------------------
0.0.8	 19/07/13  Inaki	 Añadir función exptimeHMinterval para intervalos (10:30-11:15)
...
/**********************************************************/
//Inaki aldaketa dena minutuetan irakurtzeko
CtI LangEU_TextToList::exptimeHM(CtI p, BOOL NONDELETE)
{
	INT hours;
	INT minutes;
	BOOL dekd;
	...
	//fprintf(stderr,"hours=%d\tminutes=%d dekd=%d\n", hours, minutes, dekd);
	...
	switch(hours)	{
		...
		case 2:
            ...
            if ((!dekd || minutes)) p = ct.extendStr(p, eu_akStr);
 			break;
		case 11:
            p = upTo99(hours, p);
		...
		case 4:
			p = upTo99(hours, p);
           // if ((!dekd || minutes)&&minutes!=30) p = ct.extendStr(p, eu_rakStr);
            if ((!dekd || minutes)) p = ct.extendStr(p, eu_rakStr);
 			break;
...
/**********************************************************/
/**********************************************************/
CtI LangEU_TextToList::exptimeHMinterval(CtI p)	//inaki para intervalos de hora 10:30-11:15
{
	CtI q,k;
	ct.setDekd(p);//marcarmos como palabra declinada
	q=ct.next(p);
	q=ct.next(q);
	//ct.setDekd(q);//marcarmos como palabra declinada
	q=ct.insafter(q, "etatik", FALSE);
	//strcat(ct(q).str, "-etatik");// añadimos la declinación a la primera hora
	//ct.setDekd(q);//marcarmos como palabra declinada
	q=exptimeHM(p, FALSE);
	for(q=ct.next(q);q != NULL; q=ct.next(q)){
		if(ct(q).str[0] == '-'){
			q=ct.del(q);
			break;
		}
