...
// READ INPUT ARGUMENTS
	...
	KVStrList pro("InputFile=input.txt OutputFile=Output.wav DataPath=data_tts Speed=100 SetDur=n help=n");
	StrList files;
	...
	clargs2props(argc, argv, pro, files,
			"InputFile=s OutputFile=s  DataPath=s Speed=s help=b SetDur=b");

	//Read the values of the input arguments
	if (pro.bval("help")){
		printf("usage: ./tts -InputFile=input.txt -OutputFile=Output.wav -DataPath=data_tts -Speed=100\n");
		return -1;
	}
	...
	//Language: Northern dialect of Basque
	const char *lang = "eu";
...
// OTHER CONFIGURATIONS (after the tts object is created)
	...
	//MODIFY THE SPEED-RATE OF THE VOICE
		...
		if(f>=25 && f<=300){
			...
			//}
			delete []tmp_speed;
		}else{fprintf(stderr,"WARNING: parameter -Speed=%d ignored\n\tits value must be an integer between 25 and 300\n",f);}
	}
//SET TRANSCRIPTION RULES FOR NORTHERN BASQUE
...
// For Basque it reads the text from a string
