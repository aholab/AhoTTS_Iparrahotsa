...
-------  --------  --------  -----------------------
0.0.1	 10/11/06  inaxio    Añadido el euro.
...
BOOL TextToList::isUnit( CtI p )
{
...
#ifdef HTTS_DEBUG
	if (!ct(p).pattern) htts_error("TextToList::isUnit() - Trying to read null Pattern.");
#endif
	...
	if (strlen(pattern) == 1)	{
		uniStr = (ct.getStr(p))[0];
		if (uniStr == '$') retval = TRUE;
		if (uniStr == '£') retval = TRUE;
		if (uniStr == '¥') retval = TRUE;
		if (uniStr == '¤') retval = TRUE;//inaxio
	}
