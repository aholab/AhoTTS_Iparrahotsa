...
-------  --------  --------  -----------------------
0.0.5	 09/09/14	Inaki	  Set para sacar o no labels de parrafo (Ha de estar el define HTTS_PARAGRAPH activado)
0.0.4	 09/10/13	Inaki	  Añadir función sentence_position_in_paragraph
0.0.3	 05/05/13	Inaki	  Añadir ForcePitch para forzar la predicción de pitch de Ahotts sobre hts_engine
...
protected:
   ...
   HTS_Boolean paragraph_info;
   HTS_Boolean force_pitch;// inaki : forzar el pitch predicho por el módulo prosódico, también activa phoneme_alignment
   double speech_speed;
   HTS_Boolean use_log_gain;
   double  interp_weight; //inaki, se lo aplicamos a todo, podría hacerse por cada stream
...
public:
  ...
  ~HTS_U2W ( );
   virtual BOOL create (const char * lang);
	//FUNCIONES
   BOOL xinput_labels (String labels);
...
private:
	...
	int sentence_position_in_paragraph(UttPh *u, Lix p);
};
#endif
#endif
