...
-------  --------  --------  -----------------------
1.0.2	 23/01/14  Inaki	 vocales francesas iparralde en IsVowel
1.0.1	 05/05/13  Inaki	 Silabificación de diptongos para dialecto de Iparralde
...
BOOL LangEU_PhUti::phIsVowel(Phone ph)
{
	return ((ph == PHEU_a) || (ph == PHEU_e) || (ph == PHEU_i)
		...
		|| (ph == PH_aa_) || (ph == PH_e_) || (ph == PH_o_)						//Iparralde
		|| (ph == PH_y) || (ph == PH_2)	;	//Iparralde
}
...
BOOL LangEU_PhUti::phIsValidCC(Phone ph1, Phone ph2)
{
	...
	if ((ph2 == PHEU_r)||(ph2 ==PHEU_rr) || ph2 == PH_R)
...
BOOL LangEU_PhUti::phIsDiptongo(Phone ph1, Phone ph2, Stress acc1, Stress acc2, BOOL phtiparralde)
{
	BOOL iu1, iu2;
	...
	if(phtiparralde && ph1 == PHEU_uaprox && ph2 == PHEU_e) //inaki w+e --> Iparralde
		return TRUE;
