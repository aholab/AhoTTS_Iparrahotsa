...
NumType LangEU_TextToList::isNum(CtI p)
{
	...
	patlen = strlen(pattern);
	//fprintf(stderr,"len=%d pattern=%s\n", patlen, pattern);
	...
	if (patlen == 3)
	{
		if ((pattern[0] == 'n') && (pattern[1] == 'p')&& (pattern[2] == 'n'))
		{
			...
			//inaki: partiduen emaitzan irakurtzeko
			else if (strcmp(ct.getStr(ct.next(p)), "-") == 0)
			{
				numTemp = NUM_RESULT_SPORT;
				retval = numTemp;
				free(pattern);
				return retval;
			}
	...
	///////////////
	//inaki Ordu tartea: 10:30-11:15
	if (patlen == 7 && !strcmp(pattern, "npnpnpn")){
