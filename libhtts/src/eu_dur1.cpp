...
PRIVATE DOUBLE dur_pause( Phone ph, Pause pautype )
{
	...
	switch (pautype) {
	case UPAUSE_UBEGIN:
	case UPAUSE_UEND: l=300; break;

	case UPAUSE_SHORT: l=150; break; //inaki 70
	case UPAUSE_LONG: l=200; break; //inaki 90

	case UPAUSE_SDECL:
	case UPAUSE_SQUEST:
	case UPAUSE_SEXCL:
	case UPAUSE_SPAUSE:
	case UPAUSE_SNEUTR: l=200; break; //inaki 80

	default: l=100; break; //inaki 30 //  aqui entran los silencios que van alrededor del PH_breath
	}
...
PRIVATE VOID utt_dur_unnorm(UttPh & ut)
{
	...
	for (p = ut.phoneFirst(); p != 0; p = ut.phoneNext(p)) {
		...
		case PH_breath:
				//////////////
				//inaki parraforako pausa luzea
				if(p == ut.phoneLast(p) && ut.cell(ut.wordFirst(p)).getParagraphPosition() < 0 ){ //paragrafoaren azken esaldia
					d=1000;
				}

				////////////////
				///inaki pausas mas cortas si solo hay una palabra entre pausas (excluyendo las iniciales/finales )
				else if(ut.cell(p).getPause() == UPAUSE_SPAUSE && ut.wordThis(ut.phonePrev(p)) == ut.wordFirst(ut.phonePrev(p), URANGE_PAUSE) && ut.wordThis(ut.phonePrev(p)) != ut.wordFirst(ut.phonePrev(p), URANGE_UTT)) {
					d=100;
				}
				...
				//////////////
					d=dur_pause(ut.cell(p).getPhone(),ut.cell(p).getPause());
