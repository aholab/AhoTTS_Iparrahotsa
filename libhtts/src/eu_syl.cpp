...
-------  --------  --------  -----------------------
1.0.1	 05/05/13  Inaki	 Silabificación de diptongos para dialecto Iparralde
...
#define SPREV(p) u.phonePrev(p,URANGE_SYLLABLE)
	...
	for (i = wordp; i != 0; i = NEXT(i)) {
		...
		if (NOESVOCAL(ph2)) {	// CV
			...
			if (LangEU_PhUti::phIsDiptongo(ph2, ph, ACC(i2), ACC(i), phtiparralde)) //inaki, si es Iparralde, w+e --> diptongo
