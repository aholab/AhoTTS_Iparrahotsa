...
-------   --------  --------  -----------------------
0.0.15	  27/05/13  Inaki	  Modificar función isThereAnUtt para evitar que se quede bloqueada
...
UttWS *TextToList::ct2ws(BOOL *flush)
{
	...
	for (p = ct.first(); !((p == lastToNoP) || endParse); p = ct.next(p)) {
		...
		if (isPuntt(p))
		{
			...
			switch((ct(p).str)[0]){
				...
				case '»':
					...
					break;
				case '"':
				//	nt->cell(sent).setSentence(USENTENCE_NONE);
					//nt->cell(sent).setPhrase(TRUE);
					//nt->cell(sent).setSentence(USENTENCE_PAROPEN);
					qq = nt->cellAppend();
					insertPause(nt, qq,UPAUSE_SHORT);
					//nt->cell(sent).setSentence(infSent(p));
					nt->cell(sent).setSentence(USENTENCE_PAUSE);
					sentTypePend = TRUE;
					//ct.setStatus(ct.next(p), ESCSEQ);
					endParse = FALSE;
				//	nt->foutput(stderr);
					continue;
					break;
			...
			switch (infSent(p)) {
				...
				case USENTENCE_SUSPEN:
					//inaki //mirar si ha habido salto de carro: fin de parrafo
				if( ct.getParagraphPosition(p) == 1){

					nt->cell(q_first_word).setParagraphPosition(-sentence_counter);
					sentence_counter = 1;
				}
				else{
					nt->cell(q_first_word).setParagraphPosition(sentence_counter);
		...
		{
			//fprintf(stderr, "ENTRAR1\n");
			if (ct.isEscSeq(p))
			{
			//	fprintf(stderr, "ENTRAR2\n");
				INT value;
			...
			//	fprintf(stderr, "ENTRAR3\n");
				pCHAR tempStr;
				tempStr = strdup(ct.getStr(p));

				if (strlen(tempStr) != 0)
				{
					//si esta marcada con transcripción en el diccionario, se mantiene la entrada inicial. Si no, se convierte a minúsculas
					//if(nt->cell(p).queryPOS(0x010000)){
					HDicRef myref = dic->search(ct(p).str);
					int len = dic->query(myref, HDIC_QUERY_MATCHLEN);
					if(!(dic->query(myref,HDIC_QUERY_EU_TF_MRK) == 1 && len == 0))
						ISOLatin1_StrLower(tempStr);
					else if( dic->query(myref,HDIC_QUERY_EU_TF_MRK) == 1 && len != 0){
						char decli[100];
...
pCHAR TextToList::ct2txt(BOOL *flush)
{
  ...
  for (p = ct.first(); !((p == lastToNoP) || endParse); p = ct.next(p)) {
      ...
      if (mustInsertSpace(p)) nttxt = concatStr(nttxt, " ");
...
BOOL TextToList::isThereAnUtt( VOID )
{
  ...
  for (p = ct.firstGrp(); !((p == lastToNoP) || endParse); p = ct.nextGrp(p)) {
     ...
     //if (ct.isFlush(p))  {
	//Inaki: si no existe siguiente grupo, salir de la función. En euskera para la entrada "7,8%" se quedaba bloqueada
	//en la función TextToList::output
     if (ct.isFlush(p)){// || ct.nextGrp(ct.firstGrp()) == NULL) {
       retval = TRUE;
       endParse = TRUE;
     }
