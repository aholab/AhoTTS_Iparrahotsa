...
BOOL LangEU_TextToList::isApost( CtI p )
{
	BOOL retval = FALSE;
	pCHAR pattern;
	...
	CHAR separator;
        ...
        if (!ct(p).pattern) htts_error("LangEU_TextToList::isApost() - Trying to read null Pattern.");
#endif
...
CtI LangEU_TextToList::expApost( CtI p )
{
	CtI q;
	INT nint = 0 ,i;
	INT patlen;
